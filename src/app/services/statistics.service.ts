...
export class StatisticsService {
  ...
  /*********************************************/
  public getSpiderData(): Observable<any> {
      ...
      this.wannabeDAO.fetchDraftedPlayers().subscribe((players: DraftedPlayerRecord[]) => {
        this.wannabeDAO.fetchTeams().subscribe((teams: OwnerRecord[]) => {
          for (const team of teams) {
            ...
            const QBs = players.filter(x => x.position === 'QB' && x.ownerName === team.teamName).sort(sortOnFantasy).reverse().slice(0, 2);
            const RBs = players.filter(x => x.position === 'RB' && x.ownerName === team.teamName).sort(sortOnFantasy).reverse().slice(0, 3);
            const DSTs = players.filter(x => x.position === 'DST' && x.ownerName === team.teamName).sort(sortOnFantasy).reverse().slice(0, 2);
            const Ks = players.filter(x => x.position === 'K' && x.ownerName === team.teamName).sort(sortOnFantasy).reverse().slice(0, 2);
            const rec = players.filter(x => (x.position === 'WR' || x.position === 'TE') && x.ownerName === team.teamName)
              .sort(sortOnFantasy).reverse().slice(0, 4);
  ...
  /*********************************************/
  public getPowerRankings(): Observable<any> {
    const rankings = [];
